...
export async function GET() {
  try {
    ...
    // Find AHENEFIE service (look for service with name containing "AHENEFIE")
    const ahenefieService = pricingData.find(item =>
      item.type === 'service' &&
      (item.name.toUpperCase().includes('AHENEFIE') || item.id.toLowerCase().includes('ahenefie'))
    );

    if (!ahenefieService) {
      return NextResponse.json({
        success: false,
        error: 'AHENEFIE service not found in pricing data'
      }, { status: 404 });
    }

    // Transform to service format
    const serviceData = transformToServiceFormat(ahenefieService);
