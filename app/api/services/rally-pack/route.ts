...
export async function GET() {
  try {
    ...
    // Find Rally Pack service (prioritize exact match)
    let rallyPackService = pricingData.find(item => 
      item.type === 'service' && 
      (item.name.toUpperCase().includes('RALLY') && 
       item.name.toUpperCase().includes('PACK')) ||
      (item.name.toUpperCase().includes('RALLY'))
    );
    
    // If not found, try broader patterns
    if (!rallyPackService) {
      rallyPackService = pricingData.find(item => 
        item.type === 'service' && 
        (item.name.toUpperCase().includes('RALLY') ||
         item.name.toUpperCase().includes('DEMONSTRATION') ||
         item.name.toUpperCase().includes('CROWD') ||
         item.name.toUpperCase().includes('PACK'))
      );
    }
    ...
    if (!rallyPackService) {
      return NextResponse.json({
        success: false,
        error: 'Rally Pack service not found in pricing data'
      }, { status: 404 });
    }
    
    // Transform to service format
    const serviceData = transformToServiceFormat(rallyPackService);
    ...
    console.error('Error fetching Rally Pack service data:', error);
    return NextResponse.json({
      success: false,
