import { Logo } from "@/lib/types/logos";

// Partner/Client Logos Configuration
// To add real logos:
// 1. Add your logo files to public/images/logos/
// 2. Update the src paths below to point to your actual logo files
// 3. Adjust width/height as needed for your logos

export const PARTNER_LOGOS: Logo[] = [
  {
    name: "UTB",
    src: "/images/logos/utb.png", // Replace with actual logo path
    alt: "UTB - University of Technology and Business",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 1,
  },
  {
    name: "NOVA",
    src: "/images/logos/nova.png", // Replace with actual logo path
    alt: "NOVA Corporation",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 2,
  },
  {
    name: "Pastosa",
    src: "/images/logos/pastosa.png", // Replace with actual logo path
    alt: "Pastosa Company",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 3,
  },
  {
    name: "Biobe",
    src: "/images/logos/biobe.png", // Replace with actual logo path
    alt: "Biobe Solutions",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 4,
  },
  {
    name: "Rabito",
    src: "/images/logos/rabito.png", // Replace with actual logo path
    alt: "Rabito Services",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 5,
  },
  {
    name: "District 4",
    src: "/images/logos/district4.png", // Replace with actual logo path
    alt: "District 4 Organization",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 6,
  },
  {
    name: "Labianca",
    src: "/images/logos/labianca.png", // Replace with actual logo path
    alt: "Labianca Company",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 7,
  },
  {
    name: "Polytank",
    src: "/images/logos/polytank.png", // Replace with actual logo path
    alt: "Polytank Industries",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 8,
  },
  {
    name: "Woodify",
    src: "/images/logos/woodify.png", // Replace with actual logo path
    alt: "Woodify Solutions",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 9,
  },
  {
    name: "Decorzone",
    src: "/images/logos/decorzone.png", // Replace with actual logo path
    alt: "Decorzone Design",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 10,
  },
];

// Fallback logos using CSS-generated placeholders (no external dependencies)
export const PLACEHOLDER_LOGOS: Logo[] = [
  {
    name: "UTB",
    src: "data:image/svg+xml;base64,",
    alt: "UTB Logo",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 1,
  },
  {
    name: "NOVA",
    src: "data:image/svg+xml;base64,",
    alt: "NOVA Logo",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 2,
  },
  {
    name: "Pastosa",
    src: "data:image/svg+xml;base64,",
    alt: "Pastosa Logo",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 3,
  },
  {
    name: "Biobe",
    src: "data:image/svg+xml;base64,",
    alt: "Biobe Logo",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 4,
  },
  {
    name: "Rabito",
    src: "data:image/svg+xml;base64,",
    alt: "Rabito Logo",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 5,
  },
  {
    name: "District 4",
    src: "data:image/svg+xml;base64,",
    alt: "District 4 Logo",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 6,
  },
  {
    name: "Labianca",
    src: "data:image/svg+xml;base64,",
    alt: "Labianca Logo",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 7,
  },
  {
    name: "Polytank",
    src: "data:image/svg+xml;base64,",
    alt: "Polytank Logo",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 8,
  },
  {
    name: "Woodify",
    src: "data:image/svg+xml;base64,",
    alt: "Woodify Logo",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 9,
  },
  {
    name: "Decorzone",
    src: "data:image/svg+xml;base64,",
    alt: "Decorzone Logo",
    width: 120,
    height: 60,
    isActive: true,
    sortOrder: 10,
  },
];

// Use this to switch between real logos and placeholders
export const USE_PLACEHOLDER_LOGOS = true; // Set to false when you have real logos

export const getPartnerLogos = (): Logo[] => {
  return USE_PLACEHOLDER_LOGOS ? PLACEHOLDER_LOGOS : PARTNER_LOGOS;
};

// API-based logo fetching (for dynamic management)
export const fetchPartnerLogos = async (): Promise<Logo[]> => {
  try {
    const response = await fetch("/api/admin/logos?active=true");
    const data = await response.json();

    if (data.success && Array.isArray(data.data)) {
      return data.data;
    } else {
      // Fallback to static logos if API fails
      return getPartnerLogos();
    }
  } catch (error) {
    console.error("Failed to fetch logos from API:", error);
    // Fallback to static logos if API fails
    return getPartnerLogos();
  }
};
