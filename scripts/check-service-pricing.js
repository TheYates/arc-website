...
async function checkServicePricing() {
  try {
    ...
    for (const service of services) {
      console.log(`📋 SERVICE: ${service.name} (${service.serviceItems.length} items)`);
      console.log(`   Base Price: ${service.basePrice ? `GHS ${service.basePrice}` : 'Not set'}`);
      ...
      if (service.serviceItems.length === 0) {
        ...
        for (const item of service.serviceItems) {
          const priceInfo = item.basePrice ? `GHS ${item.basePrice}` : 'NO PRICE';
          ...
          console.log(`   ${levelIndent}   └─ ${requiredStatus} | ${priceInfo} | Level ${item.level}`);
          
          if (item.description) {
            console.log(`   ${levelIndent}      Description: ${item.description.substring(0, 60)}...`);
          }
        }
      }
    ...
    // Summary statistics
    const totalItems = services.reduce((sum, service) => sum + service.serviceItems.length, 0);
    const optionalItems = services.reduce((sum, service) => 
      sum + service.serviceItems.filter(item => !item.isRequired).length, 0);
    const optionalItemsWithPrice = services.reduce((sum, service) => 
      sum + service.serviceItems.filter(item => !item.isRequired && item.basePrice && item.basePrice > 0).length, 0);
    ...
    console.log(`   Optional items WITHOUT price: ${optionalItemsWithoutPrice}`);
